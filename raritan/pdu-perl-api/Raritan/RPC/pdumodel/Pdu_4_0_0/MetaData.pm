...
package Raritan::RPC::pdumodel::Pdu_4_0_0::MetaData;
...
sub encode {
    ...
    $encoded->{'isEnergyPulseSupported'} = ($in->{'isEnergyPulseSupported'}) ? JSON::true : JSON::false;
    return $encoded;
}
...
sub decode {
    ...
    $decoded->{'isEnergyPulseSupported'} = ($in->{'isEnergyPulseSupported'}) ? 1 : 0;
    return $decoded;
}
