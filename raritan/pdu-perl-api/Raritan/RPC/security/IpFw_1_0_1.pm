...
package Raritan::RPC::security::IpFw_1_0_1;
...
sub encode {
    ...
    for (my $i0 = 0; $i0 <= $#{$in->{'ruleSet'}}; $i0++) {
        $encoded->{'ruleSet'}->[$i0] = Raritan::RPC::security::IpfwRule::encode($in->{'ruleSet'}->[$i0]);
    }
    return $encoded;
}
...
sub decode {
    ...
    $decoded->{'ruleSet'} = [];
    for (my $i0 = 0; $i0 <= $#{$in->{'ruleSet'}}; $i0++) {
        $decoded->{'ruleSet'}->[$i0] = Raritan::RPC::security::IpfwRule::decode($agent, $in->{'ruleSet'}->[$i0]);
    }
    return $decoded;
}
