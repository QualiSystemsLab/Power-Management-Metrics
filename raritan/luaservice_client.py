...
# command line args
parser = argparse.ArgumentParser(prog="luaservice_client", 
								 description="Manage Lua scripts")

mutable_grp = parser.add_mutually_exclusive_group()
mutable_grp.add_argument("-r", "--run", help="set script, start script, show output", 
						 type=argparse.FileType('r'))
mutable_grp.add_argument("-f", "--file", help="to set a default script",
						 type=argparse.FileType('r'))
addr_grp = parser.add_argument_group("pdu address")
addr_grp.add_argument("-a", "--addr", help="an address or ip address of the pdu", 
					  required=True)
